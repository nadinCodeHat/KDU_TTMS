...
{
    public partial class Lecturer_View_Students : Form
    {
        public Lecturer_View_Students()
        {
            InitializeComponent();
        }
        ...
        private void loadIntakes()
        {
            ...
            while (reader.Read())
            {
                intake_combo.Items.Add(reader["intake"]);
            }
            reader.Close();
            databaseConnection.Close();
            intake_combo.SelectedIndex = 0;
        }


        private void intake_combo_SelectedIndexChanged(object sender, EventArgs e)
        {
            LoadStudentsAll(intake_combo.SelectedItem.ToString()); ;
        }
